({
    ...
    initializeHeader: function(component, event, helper) {
    ...
    handleDecrement: function(component, event, helper) {
        let totalMinutes = component.get('v.record.TimeTaken__c')
        let hours = 0;
        let curMins = 0;
        if (totalMinutes != 0) {
            ...
            if (totalMinutes === 0) {
                $A.util.removeClass(component.find('expId'), 'slds-timeline__item_task');
                $A.util.addClass(component.find('expId'), 'slds-timeline__item_event');
        ...
        component.set('v.record.TimeTaken__c', totalMinutes);
    },
    ...
    handleHourIncrement: function(component, event, helper) {
        ...
        if (totalMinutes >= 0 && totalMinutes < 480) {
            if (totalMinutes === 0) {
                ...
                $A.util.removeClass(component.find('expId'), 'slds-timeline__item_event');
                $A.util.addClass(component.find('expId'), 'slds-timeline__item_task');
    ...
    handleOffLocationClick: function(component, event) {
        var cmpEvent = component.getEvent("viewOffLocation");
        cmpEvent.setParams({ "firedFrom": "ViewOffLocation" });
        cmpEvent.fire();
    },
