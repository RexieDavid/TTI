({
    ...
    populateForEdit: function(component){
        ...
        if (asset.Receipt__c == null) {
            ...
            isBunningsRec = asset.Receipt__r.ReceiptSource__c === 'Bunnings' ? "Yes" : "No";
        }
        component.set("v.isBunningReceipt", isBunningsRec);
    },
    
    loadReceipt: function(component, asset) {
        //Could be the case when created via customer service
        if (asset.Receipt__c == null) {
            var receiptName ='';
        } else {
    ...
    displayServerErrorMessage: function(component, response) {
        this.hideprogress(component);
        this.showErrorMessage(component, "Oops...something went wrong. Please try again, if error continues contact our customer support." + response.getError());
    },
